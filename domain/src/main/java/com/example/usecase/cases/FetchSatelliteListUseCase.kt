...
class FetchSatelliteListUseCase @Inject constructor(
    ...
    override fun abraKadabra(): Flow<Magic<List<SatelliteListItemObject>>> = flow {
        ...
        emit(Magic.loading())
        if (name.length > 2) {
            val data = satelliteRepository.fetchSatelliteList().toList().filter {
                it.name.contains(other = name, ignoreCase = true)
            }
            emit(Magic.success(data))
        } else if (name.isEmpty()) {
            val data = satelliteRepository.fetchSatelliteList().toList()
            emit(Magic.success(data))
        }
    }.catch {
